...
import second.lab.bits.AdditionalModuloOperator;
import second.lab.bits.BitArray;
import second.lab.bits.ExclusiveOrOperator;
import second.lab.bits.MultiplicationModuloOperator;
...
public class Round {
    public TextBlock encrypt(TextBlock block,
        ...
        if (k1.size() != 16 || k2.size() != 16 || k3.size() != 16 ||
                k4.size() != 16 || k5.size() != 16 || k6.size() != 16) {
            throw new IllegalArgumentException();
        }

        TextBlock resultBlock = block.cloneDeep();

        AdditionalModuloOperator additionModuloOperator = new AdditionalModuloOperator();
