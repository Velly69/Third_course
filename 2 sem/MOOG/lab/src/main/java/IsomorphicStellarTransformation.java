...
public class IsomorphicStellarTransformation extends JFrame
{
    ...
    // Конструктор, де викликаються всі основні функції та виводиться результат виконання програми
    public IsomorphicStellarTransformation(int n, int xZ1, int yZ1, int angleZ1)
    {
        ...
        Z1 = new ArrayList<>();
        Z2 = new ArrayList<>();
        initStellatedPolygon(xZ1, yZ1, radius, angleZ1, Z1);
        initStellatedPolygon(xZ2, yZ2, radius, angleZ2, Z2);
        findTransformation();
    ...
    // Функція для показу многокутників
    @Override
    public void paint(Graphics g)
    {
        ...
        if (!rep){
            Z1draw = drawStellatedPolygon(xZ1, yZ1, radius, angleZ1, g, Color.BLUE, 1);
            //Z2draw = drawStellatingPolygon(xZ2, yZ2, radius, angleZ2, g, Color.RED, 2);
            Z2draw = drawStellatedPolygon(xZ1, yZ1, radius*2, angleZ1+30,  g, Color.RED, 2);
            rep = true;
        } else {
    ...
    // Функція для перетворення координати х для більш точного виведення на екран
    private int convertX(double x){
        double x_log = x / 100;
        return (int)(margin + (1.0 / 2) * (x_log + 1) * (width - 2 * margin));
    }

    // Функція для перетворення координати у для більш точного виведення на екран
    private int convertY(double y){
        double y_log = y / 100;
        return (int) (margin + (-1.0 / 2) * (y_log - 1) * (height - 2 * margin));
    }

    // Функція для задання точок зіркового многокутника
    private void initStellatedPolygon(int x0, int y0, int radius, int turn, ArrayList<Point.Double> polygon){
    ...
    // Функція для зображення зіркового многокутника
    private ArrayList<Point.Double> drawStellatedPolygon(int x0, int y0, int radius, int turn, Graphics graphics, Color color, int num){
    ...
    // Функція для пошуку ізоморфного перетворення зіркових многокутників Z1 в Z2
    private void findTransformation(){
        ...
        double dividerDeterminant = divider[0][0] * divider[1][1] * divider[2][2]
        ...
        x[0] = (sum - differ) / dividerDeterminant;
        ...
        x[1] =  (sum - differ) / dividerDeterminant;
        ...
        y[0] =  (sum - differ) / dividerDeterminant;
        ...
        y[1] =  (sum - differ) / dividerDeterminant;
        ...
        trans[0] =  (sum - differ) / dividerDeterminant;
        ...
        trans[1] =  (sum - differ) / dividerDeterminant;


        results  = new ArrayList<>();
        results.add("<html><p style='padding-left: 40px; font-size: 15px'>Isomorphic Stellar Transformation " + n
                + "-angles <font color='blue'>Z1</font> to <font color='red'>Z2</font> looks like: </p></html>");
        results.add("");
        ...
        results.add("");
        results.add("<html><p style='padding-left: 120px; font-size: 12px'>Angle of rotation: " + Math.round(Math.acos(x[0]) * 180 / Math.PI) + "&deg</p></html>");
        results.add("");
        results.add("<html><p style='padding-left: 120px; font-size: 12px'>Translation: (" + Math.round(trans[0] * 100) / 100.0
