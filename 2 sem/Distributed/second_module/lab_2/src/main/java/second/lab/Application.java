...
public class Application extends JFrame {
    ...
    private static void save() {
        if (editMode) {
            if (genreMode) {
                ...
                if (!GenreDAO.update(currentGenre)) {
                ...
                if (!MovieDAO.update(currentMovie)) {
                    JOptionPane.showMessageDialog(null, "Error: update failed!");
                }
            }
        } else {
    ...
    private static void delete() {
        if (editMode) {
            if (genreMode) {
                ...
                GenreDAO.delete(currentGenre);
                comboGenre.removeItem(currentGenre.getName());
            } else {
                MovieDAO.delete(currentMovie);
    ...
    private static void fillMovieFields() {
        if (currentMovie == null)
            return;
        GenreDTO genre = GenreDAO.findById(currentMovie.getGenreId());
