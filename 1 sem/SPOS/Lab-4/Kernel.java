...
public class Kernel extends Thread {
  ...
  public static int matrix[][] = new int[virtPageNum+1][virtPageNum+1];
  ...
  public void init(String commands, String config) {
    ...
    if ( config != null ) {
      ...
      for(int x = 0; x <= virtPageNum; x++){
        for(int y = 0; y <= virtPageNum; y++){
          matrix[x][y] = 0;
        }
      }
      try {
    ...
    for (i = 0; i < virtPageNum; i++) {
      ...
      for (j = 0; j < virtPageNum; j++) {
        Page tmpPage = memVector.elementAt(j);
        if (tmpPage.physical == page.physical && page.physical >= 0) {
          physical_count++;
        }
      }
      if (physical_count > 1) {
  ...
  public void step() {
    ...
    if (instruct.inst.startsWith("READ")) {
      ...
      if (page.physical == -1) {
        ...
        PageFault.replacePage(memVector, virtPageNum, Virtual2Physical.pageNum(instruct.addr, virtPageNum, block), controlPanel, matrix);
        controlPanel.pageFaultValueLabel.setText("YES");
      } else {
        //page.R = 1;
        modifyBitMatrix(page.id);
  ...
  private void modifyBitMatrix(int pageId) {
