...
//  }
  ...
  public static void replacePage(Vector<Page> mem, int virtPageNum, int replacePageNum, ControlPanel controlPanel, int [][] matrix){
    int index = 0;
    int minBinaryValue = Integer.MAX_VALUE;
    for(int i = 0; i <= virtPageNum; i++){
      int rowValue = 0;
      for (int j = 0; j <= virtPageNum; j++) {
        rowValue+= matrix[i][j] * Math.pow(2, j);
      }
      if(rowValue < minBinaryValue && mem.elementAt(i).physical != -1){
        minBinaryValue = rowValue;
        index = i;
      }
    }
